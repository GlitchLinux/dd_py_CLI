...
class DDUtilityCLI:
    ...
    def create_new_partition(self):
        print("\n\033[38;5;201mCreate New Partition\033[0m")
        print(" ")
        
        print("\033[38;5;201m1.\033[0m \033[38;5;82mNew partition table and partition\033[0m")
        print("\033[38;5;201m2.\033[0m \033[38;5;82mCreate new partition on existing disk\033[0m")
        choice = input("\n\033[38;5;82mSelect option \033[0m\033[38;5;201m(1-2)\033[0m\033[38;5;82m: \033[0m").strip()
    ...
    def create_partition_table_and_partition(self):
        """Create new partition table and first partition"""
        disk = self.select_disk("Select disk to create partition table on")
        if not disk:
            return
        ...
        fs_choice = input("\n\033[38;5;82mSelect filesystem \033[0m\033[38;5;201m(1-8)\033[0m\033[38;5;82m: \033[0m").strip()
        selected_fs = None
        ...
        if not self.confirm_operation(
            ...
            f"3. Format it as {selected_fs.upper()}\n"
            f"WARNING: This will destroy all data on this disk!"
        ):
            return
        ...
        try:
            ...
            if table_type == "msdos":
                ...
                subprocess.run(
                    ["sudo", "parted", "-s", disk, "mkpart", "primary", selected_fs, "0%", f"{size_mb}MB"],
                    check=True
                )
            else:
                # For GPT, just create partition
                subprocess.run(
                    ["sudo", "parted", "-s", disk, "mkpart", selected_fs, "0%", f"{size_mb}MB"],
                    check=True
                )
            ...
            if len(partitions) > 1:
                # Clean the partition name by removing special characters
                new_partition = self.clean_partition_name(partitions[1].strip())
                
                # Format the partition
                print(f"\033[38;5;201mFormatting {new_partition} as {selected_fs}...\033[0m")
                
                if selected_fs.startswith("fat"):
                    cmd = ["sudo", "mkfs.vfat", "-F", selected_fs[3:], new_partition]
                elif selected_fs == "exfat":
                    cmd = ["sudo", "mkfs.exfat", new_partition]
                elif selected_fs == "ntfs":
                    cmd = ["sudo", "mkfs.ntfs", "-Q", new_partition]
                elif selected_fs.startswith("ext"):
                    cmd = ["sudo", f"mkfs.{selected_fs}", new_partition]
                elif selected_fs == "btrfs":
                    cmd = ["sudo", "mkfs.btrfs", "-f", new_partition]
                
                subprocess.run(cmd, check=True)
                
                print(f"\033[38;5;82mSuccessfully created and formatted {new_partition} as {selected_fs}\033[0m")
            else:
    ...
    def create_partition_on_existing_disk(self):
        """Create new partition on existing disk with free space"""
        ...
        try:
            ...
            subprocess.run(
                ["sudo", "parted", "-s", disk, "mkpart", "primary", selected_fs, f"{start}MB", f"{end}MB"],
                check=True
            )
            ...
            if len(partitions) > 1:
                # Find the newest partition (last one) and clean its name
                new_partition = self.clean_partition_name(partitions[-1].strip())
...
def main():
    ...
    while True:
        ...
        print("\033[38;5;201m8.\033[0m \033[38;5;82mCreate New Partition\033[0m")
        print("\033[38;5;201m9.\033[0m \033[38;5;82mList Disks\033[0m")
        print("\033[38;5;201m10.\033[0m \033[38;5;82mExit\033[0m")
        
        choice = input("\n\033[38;5;82mSelect operation \033[0m\033[38;5;201m(1-10)\033[0m\033[38;5;82m: \033[0m").strip()
